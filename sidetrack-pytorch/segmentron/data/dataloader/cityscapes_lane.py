...
class CitySegmentation(SegmentationDataset):
    ...
    def __init__(self, root='datasets/cityscapes', split='train', mode=None, transform=None, **kwargs):
        ...
        self.valid_classes = [0, 1]    # self.valid_classes = [0, 1, 2, 3]
        self._key = np.array([-1, 0, 1]) # self._key = np.array([-1, 0, 1, 2, 3])
    ...
    def classes(self):
        """Category names."""
        return ('unlabel', 'lane')
        # return ('unlabel', 'road', 'zebra', 'side')
        ...
        for root, _, files in os.walk(img_folder):
            for filename in files:
                ...
                if filename.endswith('.png'):
                    ...
                    maskname = filename.split('.')[0] + '_gtFine_labelIds' + '.png'
                    maskpath = root.replace('leftImg8bit', 'gtFine')
                    maskpath = os.path.join(maskpath, maskname)
